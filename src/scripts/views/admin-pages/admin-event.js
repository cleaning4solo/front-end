...
const Event = {
    ...
    return `
      ...
      <div class="form-container container color-text">
        ...
        <form id="addBlogForm">
          ...
          <label for="imageUrl" class="col-sm-3 col-form-label">Gform Url</label>
          <div class="col-sm-9 input-group">
            <input type="text" class="form-control" id="imageUrl" name="imageUrl" required>
            <div class="input-group-append">
              <span class="input-group-text h-100 "><i class='bx bx-link'></i></span>
          ...
          <button type="submit" class="btn-add-blog btn btn-success rounded-pill my-3 px-5 py-2">Add event</button>
        </form>
      </div>
  ...
  async afterRender() {
    ...
    const eventData = await Cleaning4SoloAPI.blogAPI();
    ...
    if (event.length === 0) {
      blogContainer.innerHTML = '<p class="text-center" data-aos="fade-up">Belum ada event</p>';
    } else {
      event.forEach((blog) => {
    ...
    buttonAddBlog.addEventListener("click", async (event) => {
      ...
      try {
        ...
        const updatedEvent = await Cleaning4SoloAPI.blogAPI();
        updatedEvent.event.forEach((blog) => {
          blogContainer.innerHTML += createBlogTableDataItemTemplate(blog);
        });
      } catch (error) {
      ...
      if (event.target.classList.contains("btnDeleteBlog")) {
        ...
        if (result.isConfirmed) {
          try {
            ...
            Swal.fire({
              title: "Deleted!",
              text: "Event post has been deleted.",
              icon: "success",
            });
            blogContainer.innerHTML = "";
            const updatedEvent = await Cleaning4SoloAPI.blogAPI();
